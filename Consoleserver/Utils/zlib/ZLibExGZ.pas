...
function GZCompressStr(const s: AnsiString; const fileName, comment: AnsiString;
  dateTime: TDateTime): RawByteString; overload;
...
procedure GZCompressString(var result: RawByteString;
...
function GZDecompressStr(const s: RawByteString;
...
{$IFDEF Version6Plus}
procedure GZDecompressString(var result: UnicodeString; const s: RawByteString;
  var fileName, comment: AnsiString; var dateTime: TDateTime); overload;
{$ENDIF}
function GZDecompressStr(const s: RawByteString): AnsiString; overload;

procedure GZDecompressString(var result: AnsiString;
...
function GZDecompressStreamSize(inStream: TStream;
  var fileName, comment: AnsiString; var dateTime: TDateTime): Longint;
  overload;
...
begin
  ...
  if dateTime <> 0 then
    header^.Time := DateTimeToUnix(dateTime);
...
procedure GZInitializeDecompressString(const s: RawByteString;
  var fileName, comment: AnsiString; var dateTime: TDateTime;
  var index: Integer);
var
  header: PGZHeader;
...
begin
  ...
  if (header^.Id1 <> $1F) or (header^.Id2 <> $8B) or
    (header^.Method <> Z_DEFLATED) or ((header^.Flags and GZ_RESERVED) <> 0)
  then
  begin
    raise EZDecompressionError.Create(zeDataError);
  end;

  if header^.Time <> 0 then
    dateTime := UnixToDateTime(header^.Time)
  else
    dateTime := 0;
  ...
  begin
    ...
    begin
      ...
      if (Size >= 0) and ((index + Size) <= maxIndex) then
        Inc(index, Size)
      else
        index := maxIndex + 1;
    end
  ...
  begin
    ...
    while (endIndex <= maxIndex) and (s[endIndex] <> #$00) do
      Inc(endIndex);
...
begin
  ...
  if (trailer^.Crc <> Crc) or (trailer^.Size <> Cardinal(Size)) then
  begin
    raise EZDecompressionError.Create(zeDataError);
  end;
end;
...
function GZCompressStr(const s: AnsiString; const fileName, comment: AnsiString;
  dateTime: TDateTime): RawByteString;
begin
...
function GZDecompressStr(const s: RawByteString;
  var fileName, comment: AnsiString; var dateTime: TDateTime): AnsiString;
begin
...
begin
  ...
  if (trailer.Crc <> Crc) or (trailer.Size <> Cardinal(endPosition - position))
  then
  begin
    raise EZDecompressionError.Create(zeDataError);
  end;
...
var
  ...
  dateTime: TDateTime;
begin
  inStream := TFileStream.Create(String(inFileName), fmOpenRead or
    fmShareDenyNone);
  ...
  try
    ...
    begin
      ...
      tempFileName := tempFileName +
        ChangeFileExt(ExtractFilename(inFileName), '');
    end
    else
      tempFileName := '_ZLIBEXGZ';
    ...
    if fileName <> '' then
    begin
      outFileName := ExtractFilePath(tempFileName) +
...
begin
  ...
  if FDateTime <> 0 then
    header.Time := DateTimeToUnix(FDateTime);
...
begin
  ...
  if header.Time <> 0 then
    FDateTime := UnixToDateTime(header.Time)
  else
    FDateTime := 0;
