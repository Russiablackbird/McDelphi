...
type
  ...
  public
    ...
    constructor Create(level: TZCompressionLevel; windowBits, memLevel: Integer;
...
procedure ZDecompressStringEx(var result: AnsiString;
  const s: RawByteString); overload;
...
function ZDecompressStr2(const s: RawByteString; windowBits: Integer)
  : AnsiString;
...
{$IFDEF Version6Plus}
procedure ZDecompressString2(var result: UnicodeString; const s: RawByteString;
  windowBits: Integer); overload;
{$ENDIF}
{ ** stream routines ***************************************************************************** }
...
begin
  ...
  ZCompressCheck(ZDeflateInit2(zstream, level, windowBits, memLevel, strategy));
...
function ZCompressStr(const s: AnsiString; level: TZCompressionLevel)
  : RawByteString;
begin
...
function ZCompressStrEx(const s: AnsiString; level: TZCompressionLevel)
  : RawByteString;
begin
...
var
  ...
  size: Integer;
begin
  ZCompress2(Pointer(s), Length(s), buffer, size, level, windowBits, memLevel,
    strategy);
...
var
  ...
  size: Integer;
begin
  ZCompress2(Pointer(s), Length(s) * SizeOf(UnicodeChar), buffer, size, level,
...
procedure ZCompressStringWeb(var result: RawByteString; const s: UnicodeString);
begin
  ZCompressString2(result, s, zcFastest, -15, 9, zsDefault);
end;
{$ENDIF}
...
procedure ZDecompressString(var result: UnicodeString; const s: RawByteString);
var
  buffer: Pointer;
  size: Integer;
begin
...
procedure ZInternalCompressStreamEx(zstream: TZStreamRec;
...
procedure ZInternalDecompressStreamEx(zstream: TZStreamRec;
  inStream, outStream: TStream);
var
  param: TZStreamParam;
begin
...
procedure ZInternalCompressStream(zstream: TZStreamRec;
  inStream, outStream: TStream);
const
  BufferSize = 32768;
var
...
procedure ZInternalDecompressStream(zstream: TZStreamRec;
  inStream, outStream: TStream);
const
  BufferSize = 32768;
var
...
begin
  ...
  begin
    if capacity = 0 then
      FreeMem(FBuffer)
    else if FBufferCapacity = 0 then
