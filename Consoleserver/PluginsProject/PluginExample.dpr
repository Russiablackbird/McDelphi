...
uses
  ...
  MessagePacket in 'MessagePacket.pas';
...
var
  Mess: string;
begin
Writeln('Записей в стеке'+ PlayersStack.Count.ToString);
  with AContext.Connection do
  begin

    Mess := 'This message send plugin';
    Mess := Mess + stringofchar(' ', 64 - Length(Mess));
    Delete(Mess, 65, MaxInt);
    IOHandler.Write(13);
    IOHandler.Write(0);
    IOHandler.Write(Mess);
  end;
  Result := False;
end;

function OnPosition(AContext:TIdContext;X,Y,Z:SmallInt;Mode,BId:Byte): Boolean; stdcall;
begin
  Result := False;
end;
